...
namespace WrapperApplication.Core
{
    public class Handler
    {
        ...
        {
            var asm = Assembly.LoadFrom(handlerPath);

            var attribute = asm.GetCustomAttribute<HandlerDeclarationAttribute>();

            if (attribute == null)
            {
                throw new Exception($"Missing \"{nameof(HandlerDeclarationAttribute)}\" for the handler assembly");
            }

            var handlerType = attribute.HandlerType;

            _handler = (IHandler) Activator.CreateInstance(attribute.HandlerType);

            if (_handler == null)
            {
                throw new Exception($"Handler must implement \"{nameof(IHandler)}\" interface");
            }
